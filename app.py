...
def predict_class(model, img, path='./'):
...
def detectFakeVideo(videoPath):
    ...
    for i in range(0,len(path_to_videos)):
        ...
        prediction = predict_class(model,video_dataset[i],'./')
        if prediction[0] == 1:
            print("REAL")
        else:
            print("FAKE")
...
def predict():
    if request.method == 'POST':
        ...
        return render_template('predict.html', fruit = output,prob=confidence, user_image = video_path)
    else:
